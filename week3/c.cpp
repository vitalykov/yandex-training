...
inline int HighestBit(size_t a) {
...
void CalculateRepetitions(std::vector<Number>& arr, std::vector<int>& bit_repetitions, int highest_bit) {
    for (const auto& a : arr) {
        Number a_copy = a;
        size_t i = 0;
...
void BottomUp(std::vector<Number>& arr, std::vector<int>& bit_repetitions, int highest_bit) {
...
int main() {
    ...
    std::sort(arr.begin(), arr.end(), [](Number& a1, Number& a2) {
